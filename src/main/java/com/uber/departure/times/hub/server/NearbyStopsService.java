...
public final class NearbyStopsService {
    ...
    @NotNull
    public Future<Map<StopId, Pair<Stop, Integer>>> detect(@NotNull Location location) {
        ...
        final Future<Map<StopId, Pair<Stop, Integer>>> result = Future.future();
        nearbyCellsStops.compose(s -> result.complete(filterNearbyStops(location, s)), result);
        return result;
    }

    @NotNull
    private Map<StopId, Pair<Stop, Integer>> filterNearbyStops(@NotNull Location location, @NotNull Stops stops) {
        final Map<StopId, Pair<Stop, Integer>> result = new HashMap<>();
        for (Stop s : stops) {
            final int distance = Location.distance(location, s.getLocation());
            if (distance < conf.getSearchRadius()) {
                result.put(s.getStopId(), Pair.of(s, distance));
            }
        }
        return result;
    }
